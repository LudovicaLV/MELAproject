...
import Creation.NewGrid;
import ParserProg5Farms.CSVUtils;
import ParserProg5Farms.ModelFarms;

public class AnalyserFarms {

	public static int numberRuns = TotalFarms.TotalFarms.numberOfRuns;
    ...
    public static int KernelNumberAnalyser = 1;
    public static int ModelNumberAnalyser = 50;   
		    	
	public static void main(String[] args) throws IOException {
		
	    String csvFile = TotalFarms.Writing.csvStringAnalyser(KernelNumberAnalyser, ModelNumberAnalyser);
		...
		BufferedReader br = new BufferedReader(
				new FileReader(TotalFarms.Writing.LogAnalyser(KernelNumberAnalyser, ModelNumberAnalyser)));
		...
		BufferedReader buf = new BufferedReader(
				new FileReader(TotalFarms.Writing.DataAnalyser(KernelNumberAnalyser, ModelNumberAnalyser, k)));
...
// 		for (int q=0; q< separateIntValues.size(); q++){
// 			System.out.println("round " + separateIntValues.size() + " " + k + " " + separateIntValues.get(q));
// 		}
		...
		for (int r=0; r < namesToCheck.length; r++){	
			...
			for (int t=0; t< timeValues.size(); t++){	
				//System.out.println(timeValues.get(t) + " " + index + "name " + namesToCheck[r] );
		        if (symbolProperty == ">="){
					...
					if (separateIntValues.get(t).get(index) > intProperty){
						statMC[r] = 1;		
						//System.out.println("round " + k + " " + "step " + t + " " + index + " " + separateIntValues.get(t).get(index) + " " + namesToCheck[r] + " " + statMC[r]);						
						break;
					}else{
						statMC[r] = 0;
					}
...
//        	System.out.println(namesToCheck[f]+ " is " + statMCValues[f] + " check!!");
//        }
        for (int f=0; f< statMCValues.length; f++){
        	...
        	ArrayList<Integer> ChoiceToAdd = new ArrayList<>();
        	ChoiceToAdd.add(ModelFarms.KernelNumber);
        	int addModel = NewGrid.getPosition50(ModelFarms.ModelNumber);
        	ChoiceToAdd.add(addModel);      

        	if (f == 0){
        	ModelFarms.Results.put(ChoiceToAdd, toAdd);  }else{
        	ModelFarms.Results.get(ChoiceToAdd).put(namesToCheckListArray.get(f), resultSMC);
        		
        	}
//        	System.out.println(ChoiceToAdd);
//        	System.out.println(ModelFarms.Results.get(ChoiceToAdd));
        	System.out.println(namesToCheck[f] + " -> " + valueMC + " \u00B1 " + confinterval);     	

		    String firstEntry = Integer.toString(namesToCheckListArray.get(f).get(0));
		    String secondEntry = Integer.toString(namesToCheckListArray.get(f).get(1));
            String prob = Double.toString(valueMC);
            String confint = Double.toString(confinterval);
            CSVUtils.writeLine(writer, Arrays.asList(firstEntry, secondEntry, prob, confint));
        }      
